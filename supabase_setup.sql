...
-- -------------------------------------------------------------
-- 3B. SUPPORT REQUESTS TABLE (CHAT HANDOFF)
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.support_requests (
    ...
    topic TEXT NOT NULL,
    message TEXT NOT NULL,
    source TEXT DEFAULT 'chat' CHECK (source IN ('chat', 'contact', 'manual')),
...
ALTER TABLE public.support_requests
  ...
  ADD COLUMN IF NOT EXISTS chat_history TEXT DEFAULT '',
...
WITH CHECK (
  ...
  AND char_length(name) BETWEEN 1 AND 100
  AND char_length(email) BETWEEN 3 AND 254
  AND char_length(topic) BETWEEN 1 AND 160
  AND char_length(message) BETWEEN 1 AND 5000
...
DROP POLICY IF EXISTS "Admins can view and manage all support requests" ON public.support_requests;
CREATE POLICY "Admins can view and manage all support requests"
